/*
* This header is generated by classdump-dyld 1.0
* on Saturday, May 19, 2018 at 4:52:17 PM Eastern Daylight Time
* Operating System: Version 10.2 (Build 14C92)
* Image Source: /var/containers/Bundle/Application/7AB1B6B1-534C-4BF9-A7EB-BE2BD4C13043/Snapchat.app/Snapchat
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/


@protocol SCProfilePageSectionContentCoordinating, SCSearchSectionCreating, UICollectionViewDelegate;
@interface SCProfileTabCarouselTabCreator : NSObject {

	id<SCProfilePageSectionContentCoordinating> _sectionCoordinator;
	id<SCSearchSectionCreating> _sectionCreator;
	id<UICollectionViewDelegate> _collectionViewDelegate;

}

@property (assign,nonatomic,__weak) id<UICollectionViewDelegate> collectionViewDelegate;              //@synthesize collectionViewDelegate=_collectionViewDelegate - In the implementation block
-(void)setCollectionViewDelegate:(id<UICollectionViewDelegate>)arg1 ;
-(id<UICollectionViewDelegate>)collectionViewDelegate;
-(id)createTabByType:(long long)arg1 ;
-(id)initWithSectionCoordinator:(id)arg1 sectionCreator:(id)arg2 ;
@end

