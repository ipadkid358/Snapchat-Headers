/*
* This header is generated by classdump-dyld 1.0
* on Saturday, May 19, 2018 at 4:52:17 PM Eastern Daylight Time
* Operating System: Version 10.2 (Build 14C92)
* Image Source: /var/containers/Bundle/Application/7AB1B6B1-534C-4BF9-A7EB-BE2BD4C13043/Snapchat.app/Snapchat
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <Snapchat/Snapchat-Structs.h>
#import <libobjc.A.dylib/NSCopying.h>

@class UIImage, SCSearchActionModel;

@interface SCProfileImageCellViewModel : NSObject <NSCopying> {

	UIImage* _cellImage;
	double _imageViewCellHeight;
	SCSearchActionModel* _tapActionModel;

}

@property (nonatomic,copy,readonly) UIImage * cellImage;                               //@synthesize cellImage=_cellImage - In the implementation block
@property (nonatomic,readonly) double imageViewCellHeight;                             //@synthesize imageViewCellHeight=_imageViewCellHeight - In the implementation block
@property (nonatomic,copy,readonly) SCSearchActionModel * tapActionModel;              //@synthesize tapActionModel=_tapActionModel - In the implementation block
-(SCSearchActionModel *)tapActionModel;
-(double)imageViewCellHeight;
-(UIImage *)cellImage;
-(id)initWithCellImage:(id)arg1 imageViewCellHeight:(double)arg2 tapActionModel:(id)arg3 ;
-(BOOL)isEqual:(id)arg1 ;
-(unsigned long long)hash;
-(id)description;
-(id)copyWithZone:(NSZone*)arg1 ;
@end

