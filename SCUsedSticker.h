/*
* This header is generated by classdump-dyld 1.0
* on Saturday, May 19, 2018 at 4:52:47 PM Eastern Daylight Time
* Operating System: Version 10.2 (Build 14C92)
* Image Source: /var/containers/Bundle/Application/7AB1B6B1-534C-4BF9-A7EB-BE2BD4C13043/Snapchat.app/Snapchat
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/


@protocol SCStickerProtocol;
@class NSDate;

@interface SCUsedSticker : NSObject {

	int _frequency;
	id<SCStickerProtocol> _sticker;
	NSDate* _lastUsedDate;
	NSDate* _lastDecayDate;

}

@property (nonatomic,copy,readonly) id<SCStickerProtocol> sticker;              //@synthesize sticker=_sticker - In the implementation block
@property (nonatomic,retain) NSDate * lastUsedDate;                             //@synthesize lastUsedDate=_lastUsedDate - In the implementation block
@property (nonatomic,retain) NSDate * lastDecayDate;                            //@synthesize lastDecayDate=_lastDecayDate - In the implementation block
@property (assign) int frequency;                                               //@synthesize frequency=_frequency - In the implementation block
-(NSDate *)lastDecayDate;
-(void)setLastDecayDate:(NSDate *)arg1 ;
-(id)initWithSticker:(id)arg1 date:(id)arg2 ;
-(void)setLastUsedDate;
-(id)initWithSticker:(id)arg1 date:(id)arg2 frequency:(int)arg3 ;
-(id)initWithCoder:(id)arg1 ;
-(void)encodeWithCoder:(id)arg1 ;
-(BOOL)isEqual:(id)arg1 ;
-(unsigned long long)hash;
-(int)frequency;
-(void)setFrequency:(int)arg1 ;
-(void)setLastUsedDate:(NSDate *)arg1 ;
-(NSDate *)lastUsedDate;
-(id<SCStickerProtocol>)sticker;
@end

