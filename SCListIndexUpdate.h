/*
* This header is generated by classdump-dyld 1.0
* on Saturday, May 19, 2018 at 4:51:47 PM Eastern Daylight Time
* Operating System: Version 10.2 (Build 14C92)
* Image Source: /var/containers/Bundle/Application/7AB1B6B1-534C-4BF9-A7EB-BE2BD4C13043/Snapchat.app/Snapchat
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <Snapchat/Snapchat-Structs.h>
#import <libobjc.A.dylib/NSCopying.h>

@interface SCListIndexUpdate : NSObject <NSCopying> {

	unsigned long long _oldIndex;
	unsigned long long _newIndex;

}

@property (nonatomic,readonly) unsigned long long oldIndex;              //@synthesize oldIndex=_oldIndex - In the implementation block
@property (nonatomic,readonly) unsigned long long newIndex;              //@synthesize newIndex=_newIndex - In the implementation block
-(id)initWithOldIndex:(unsigned long long)arg1 newIndex:(unsigned long long)arg2 ;
-(unsigned long long)oldIndex;
-(unsigned long long)newIndex;
-(BOOL)isEqual:(id)arg1 ;
-(unsigned long long)hash;
-(id)description;
-(id)copyWithZone:(NSZone*)arg1 ;
@end

