/*
* This header is generated by classdump-dyld 1.0
* on Saturday, May 19, 2018 at 4:52:48 PM Eastern Daylight Time
* Operating System: Version 10.2 (Build 14C92)
* Image Source: /var/containers/Bundle/Application/7AB1B6B1-534C-4BF9-A7EB-BE2BD4C13043/Snapchat.app/Snapchat
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/


@class NSString;

@interface SCSnapcodeGenerator : NSObject {

	NSString* _svgGhost;
	int _canvasSize;

}
-(id)SVGViewFromSVGString:(id)arg1 ;
-(id)SVGStringForSnapcodeGhost;
-(id)generateBitmojiSnapcodeWithUUID:(id)arg1 version:(int)arg2 borderWidth:(double)arg3 ;
-(id)SVGStringFromSnapcodeUUID:(id)arg1 version:(int)arg2 borderWidth:(double)arg3 dotsOnly:(BOOL)arg4 ;
-(id)SVGStringFromSnapcodeData:(id)arg1 version:(int)arg2 borderWidth:(double)arg3 dotsOnly:(BOOL)arg4 ;
-(id)initWithCanvasSize:(int)arg1 ;
-(id)init;
@end

