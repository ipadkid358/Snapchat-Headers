/*
* This header is generated by classdump-dyld 1.0
* on Saturday, May 19, 2018 at 4:52:09 PM Eastern Daylight Time
* Operating System: Version 10.2 (Build 14C92)
* Image Source: /var/containers/Bundle/Application/7AB1B6B1-534C-4BF9-A7EB-BE2BD4C13043/Snapchat.app/Snapchat
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <libobjc.A.dylib/NSCoding.h>

@class NSDictionary, NSString;

@interface SCArchivedGroups : NSObject <NSCoding> {

	NSDictionary* _groups;
	NSString* _selfUsername;

}

@property (readonly) NSDictionary * groups;                          //@synthesize groups=_groups - In the implementation block
@property (nonatomic,readonly) NSString * selfUsername;              //@synthesize selfUsername=_selfUsername - In the implementation block
-(NSString *)selfUsername;
-(id)initWithGroups:(id)arg1 selfUsername:(id)arg2 ;
-(id)initWithCoder:(id)arg1 ;
-(void)encodeWithCoder:(id)arg1 ;
-(NSDictionary *)groups;
@end

