/*
* This header is generated by classdump-dyld 1.0
* on Saturday, May 19, 2018 at 4:52:31 PM Eastern Daylight Time
* Operating System: Version 10.2 (Build 14C92)
* Image Source: /var/containers/Bundle/Application/7AB1B6B1-534C-4BF9-A7EB-BE2BD4C13043/Snapchat.app/Snapchat
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <Snapchat/Snapchat-Structs.h>
#import <Snapchat/SCSearchCollectionViewCell.h>
#import <Snapchat/SCSearchActionable.h>

@protocol SCSearchActionHandling;
@class UILabel, SCShapeView, SCButton, SCSearchActionButton, SCSearchActionModel, SCAvatarView, SCAvatarViewConfiguration, SCCheetahQuickAddDataSource, SCUserSession, NSString;

@interface SCCheetahQuickAddCollectionViewCell : SCSearchCollectionViewCell <SCSearchActionable> {

	UILabel* _displayNameLabel;
	UILabel* _usernameLabel;
	UILabel* _suggestedReasonLabel;
	CGRect _shapeLayerPathRect;
	SCShapeView* _backgroundShapeView;
	SCButton* _dismissButton;
	SCSearchActionButton* _addFriendButton;
	SCSearchActionModel* _dismissActionModel;
	SCAvatarView* _avatarView;
	SCAvatarViewConfiguration* _avatarViewConfiguration;
	id _viewModel;
	id<SCSearchActionHandling> _actionHandler;
	SCCheetahQuickAddDataSource* _friendProvider;
	SCUserSession* _userSession;

}

@property (nonatomic,retain) SCCheetahQuickAddDataSource * friendProvider;              //@synthesize friendProvider=_friendProvider - In the implementation block
@property (nonatomic,retain) SCUserSession * userSession;                               //@synthesize userSession=_userSession - In the implementation block
@property (readonly) unsigned long long hash; 
@property (readonly) Class superclass; 
@property (copy,readonly) NSString * description; 
@property (copy,readonly) NSString * debugDescription; 
@property (nonatomic,retain) id<SCSearchActionHandling> actionHandler;                  //@synthesize actionHandler=_actionHandler - In the implementation block
+(CGSize)sizeWithViewModel:(id)arg1 constrainedToSize:(CGSize)arg2 ;
-(void)setFriendProvider:(SCCheetahQuickAddDataSource *)arg1 ;
-(SCCheetahQuickAddDataSource *)friendProvider;
-(void)_dismissButtonClicked;
-(void)_updateRoundedCornersWithroundedRect:(CGRect)arg1 ;
-(void)_updateWithPersonViewModel:(id)arg1 ;
-(void)_updateProfileImageWithFriend:(id)arg1 ;
-(void)_updateAddFriendButtonViewModel:(id)arg1 ;
-(void)_updateLabelsWithName:(id)arg1 username:(id)arg2 suggestedReason:(id)arg3 ;
-(void)_addFriendButtonClicked;
-(id)initWithFrame:(CGRect)arg1 ;
-(void)layoutSubviews;
-(SCUserSession *)userSession;
-(void)setUserSession:(SCUserSession *)arg1 ;
-(id)viewModel;
-(void)setViewModel:(id)arg1 ;
-(void)setActionHandler:(id<SCSearchActionHandling>)arg1 ;
-(id<SCSearchActionHandling>)actionHandler;
@end

