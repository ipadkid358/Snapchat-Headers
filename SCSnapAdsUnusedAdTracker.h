/*
* This header is generated by classdump-dyld 1.0
* on Saturday, May 19, 2018 at 4:52:10 PM Eastern Daylight Time
* Operating System: Version 10.2 (Build 14C92)
* Image Source: /var/containers/Bundle/Application/7AB1B6B1-534C-4BF9-A7EB-BE2BD4C13043/Snapchat.app/Snapchat
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/


@class SCSnapAdsEventBus, NSMutableSet, NSMutableDictionary;

@interface SCSnapAdsUnusedAdTracker : NSObject {

	SCSnapAdsEventBus* _snapAdsEventBus;
	NSMutableSet* _unusedAds;
	NSMutableDictionary* _activeAds;

}

@property (nonatomic,retain) SCSnapAdsEventBus * snapAdsEventBus;              //@synthesize snapAdsEventBus=_snapAdsEventBus - In the implementation block
@property (nonatomic,retain) NSMutableSet * unusedAds;                         //@synthesize unusedAds=_unusedAds - In the implementation block
@property (nonatomic,retain) NSMutableDictionary * activeAds;                  //@synthesize activeAds=_activeAds - In the implementation block
-(NSMutableSet *)unusedAds;
-(void)setUnusedAds:(NSMutableSet *)arg1 ;
-(void)onAdResolvedEvent:(id)arg1 ;
-(void)onAdExpiredEvent:(id)arg1 ;
-(SCSnapAdsEventBus *)snapAdsEventBus;
-(void)setSnapAdsEventBus:(SCSnapAdsEventBus *)arg1 ;
-(id)initWithSnapAdsEventBus:(id)arg1 ;
-(void)registerSnapAdsEvents;
-(BOOL)onAdRequestEvent:(id)arg1 ;
-(void)onAdTrackedEvent:(id)arg1 ;
-(void)onEnterAdUnitEvent:(id)arg1 ;
-(void)onAdTrackedAndGetEvent:(id)arg1 ;
-(NSMutableDictionary *)activeAds;
-(void)setActiveAds:(NSMutableDictionary *)arg1 ;
@end

