/*
* This header is generated by classdump-dyld 1.0
* on Saturday, May 19, 2018 at 4:52:14 PM Eastern Daylight Time
* Operating System: Version 10.2 (Build 14C92)
* Image Source: /var/containers/Bundle/Application/7AB1B6B1-534C-4BF9-A7EB-BE2BD4C13043/Snapchat.app/Snapchat
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <CoreData/NSManagedObject.h>

@class SCCDCloudSyncOperationSnapshotID, NSDate, NSData, NSString, NSNumber, SCCDGalleryProfile;

@interface _SCCDCloudSyncOperationSnapshot : NSManagedObject

@property (nonatomic,readonly) SCCDCloudSyncOperationSnapshotID * objectID; 
@property (nonatomic,retain) NSDate * createTimeUtc; 
@property (nonatomic,retain) NSData * payload; 
@property (nonatomic,retain) NSString * requestID; 
@property (nonatomic,retain) NSNumber * seqNum; 
@property (assign) long long seqNumValue; 
@property (nonatomic,retain) NSString * targetEntryId; 
@property (nonatomic,retain) SCCDGalleryProfile * owner; 
+(id)entityInManagedObjectContext:(id)arg1 ;
+(id)insertInManagedObjectContext:(id)arg1 ;
+(id)keyPathsForValuesAffectingValueForKey:(id)arg1 ;
+(id)entityName;
-(long long)seqNumValue;
-(void)setSeqNumValue:(long long)arg1 ;
-(long long)primitiveSeqNumValue;
-(void)setPrimitiveSeqNumValue:(long long)arg1 ;
-(SCCDCloudSyncOperationSnapshotID *)objectID;
@end

