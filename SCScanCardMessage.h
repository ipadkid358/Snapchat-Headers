/*
* This header is generated by classdump-dyld 1.0
* on Saturday, May 19, 2018 at 4:52:48 PM Eastern Daylight Time
* Operating System: Version 10.2 (Build 14C92)
* Image Source: /var/containers/Bundle/Application/7AB1B6B1-534C-4BF9-A7EB-BE2BD4C13043/Snapchat.app/Snapchat
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <Snapchat/SCScanCardTableViewCell.h>

@class UIImage, UIImageView, UIButton, NSString, UILabel;

@interface SCScanCardMessage : SCScanCardTableViewCell {

	UIImage* _headerImage;
	UIImageView* _headerImageView;
	UIButton* _button;
	NSString* _buttonText;
	UILabel* _messageLabel;
	BOOL _shouldRenderHeaderCircle;

}
+(id)messageCardWithHeaderImage:(id)arg1 messageText:(id)arg2 ;
+(id)genericErrorCard;
+(id)messageCardWithHeaderImage:(id)arg1 messageText:(id)arg2 buttonText:(id)arg3 shouldRenderHeaderCircle:(BOOL)arg4 ;
+(id)friendYourselfCard;
+(id)alreadySubscribedCard;
+(id)alreadyFriendsCardWithFriendName:(id)arg1 ;
+(id)alreadyUnlockedStickerCard;
+(id)noProviderSetErrorCard;
+(id)noProductFoundErrorCard;
-(void)transitionToContentAnimated:(BOOL)arg1 ;
-(void)_didTapButton;
-(id)initWithHeaderImage:(id)arg1 messageText:(id)arg2 ;
-(id)initWithHeaderImage:(id)arg1 messageText:(id)arg2 buttonText:(id)arg3 shouldRenderHeaderCircle:(BOOL)arg4 ;
-(void)_updatePreferredSize;
-(void)updateStyle;
@end

