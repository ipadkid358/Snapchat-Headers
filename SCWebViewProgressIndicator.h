/*
* This header is generated by classdump-dyld 1.0
* on Saturday, May 19, 2018 at 4:51:48 PM Eastern Daylight Time
* Operating System: Version 10.2 (Build 14C92)
* Image Source: /var/containers/Bundle/Application/7AB1B6B1-534C-4BF9-A7EB-BE2BD4C13043/Snapchat.app/Snapchat
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <Snapchat/Snapchat-Structs.h>
#import <UIKit/UIView.h>

@class FBShimmeringLayer, CAGradientLayer;

@interface SCWebViewProgressIndicator : UIView {

	FBShimmeringLayer* _shimmeringLayer;
	CAGradientLayer* _barLayer;
	BOOL _needsReset;
	BOOL _animating;
	double progress;

}

@property (assign,nonatomic) double progress; 
@property (assign,getter=isAnimating,nonatomic) BOOL animating;              //@synthesize animating=_animating - In the implementation block
+(Class)layerClass;
-(id)initWithFrame:(CGRect)arg1 leftColor:(id)arg2 rightColor:(id)arg3 ;
-(void)_configMaskLayerWithHeight:(double)arg1 ;
-(void)_updateShimmeringIfNeeded;
-(void)setProgress:(double)arg1 withExplicitDuration:(double)arg2 completion:(/*^block*/id)arg3 ;
-(void)_resetProgressBar;
-(void)rotateProgressBar;
-(id)initWithFrame:(CGRect)arg1 ;
-(void)layoutSubviews;
-(void)setProgress:(double)arg1 ;
-(BOOL)isAnimating;
-(void)setAnimating:(BOOL)arg1 ;
-(double)progress;
@end

