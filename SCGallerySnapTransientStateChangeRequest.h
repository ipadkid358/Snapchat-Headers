/*
* This header is generated by classdump-dyld 1.0
* on Saturday, May 19, 2018 at 4:52:15 PM Eastern Daylight Time
* Operating System: Version 10.2 (Build 14C92)
* Image Source: /var/containers/Bundle/Application/7AB1B6B1-534C-4BF9-A7EB-BE2BD4C13043/Snapchat.app/Snapchat
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/


@class SCCDGallerySnapTransientState, SCObjectPlaceholder, NSString;

@interface SCGallerySnapTransientStateChangeRequest : NSObject {

	SCCDGallerySnapTransientState* _gallerySnapTransientState;
	SCObjectPlaceholder* _objectPlaceholder;

}

@property (nonatomic,readonly) SCObjectPlaceholder * placeholderForCreatedGallerySnapTransientState; 
@property (nonatomic,copy,readonly) NSString * objectID; 
@property (nonatomic,copy) NSString * bgMediaUploadKey; 
@property (assign,nonatomic) int bgMediaUploadState; 
@property (nonatomic,copy) NSString * snapId; 
+(id)changeRequestForGallerySnapTransientState:(id)arg1 ;
+(id)creationRequestWithGallerySnapTransientState:(id)arg1 ;
+(void)deleteGallerySnapTransientStates:(id)arg1 ;
+(void)deleteAllGallerySnapTransientStates;
-(NSString *)snapId;
-(void)setSnapId:(NSString *)arg1 ;
-(int)bgMediaUploadState;
-(void)setBgMediaUploadState:(int)arg1 ;
-(NSString *)bgMediaUploadKey;
-(void)setBgMediaUploadKey:(NSString *)arg1 ;
-(id)initWithGallerySnapTransientState:(id)arg1 ;
-(SCObjectPlaceholder *)placeholderForCreatedGallerySnapTransientState;
-(void)setWithGallerySnapTransientState:(id)arg1 ;
-(NSString *)objectID;
@end

