/*
* This header is generated by classdump-dyld 1.0
* on Saturday, May 19, 2018 at 4:52:33 PM Eastern Daylight Time
* Operating System: Version 10.2 (Build 14C92)
* Image Source: /var/containers/Bundle/Application/7AB1B6B1-534C-4BF9-A7EB-BE2BD4C13043/Snapchat.app/Snapchat
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/


@class NSError;

@interface SCDiscoverEditionMediaLoadingRecord : NSObject {

	BOOL _needsLoading;
	BOOL _finishedLoading;
	BOOL _loadingSuccess;
	NSError* _loadingError;

}

@property (assign,nonatomic) BOOL needsLoading;                   //@synthesize needsLoading=_needsLoading - In the implementation block
@property (assign,nonatomic) BOOL finishedLoading;                //@synthesize finishedLoading=_finishedLoading - In the implementation block
@property (assign,nonatomic) BOOL loadingSuccess;                 //@synthesize loadingSuccess=_loadingSuccess - In the implementation block
@property (nonatomic,retain) NSError * loadingError;              //@synthesize loadingError=_loadingError - In the implementation block
-(void)setNeedsLoading:(BOOL)arg1 ;
-(BOOL)needsLoading;
-(BOOL)finishedLoading;
-(BOOL)loadingSuccess;
-(void)setFinishedLoading:(BOOL)arg1 ;
-(void)setLoadingSuccess:(BOOL)arg1 ;
-(NSError *)loadingError;
-(void)setLoadingError:(NSError *)arg1 ;
@end

