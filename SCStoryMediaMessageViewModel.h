/*
* This header is generated by classdump-dyld 1.0
* on Saturday, May 19, 2018 at 4:52:28 PM Eastern Daylight Time
* Operating System: Version 10.2 (Build 14C92)
* Image Source: /var/containers/Bundle/Application/7AB1B6B1-534C-4BF9-A7EB-BE2BD4C13043/Snapchat.app/Snapchat
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/


#import <Snapchat/Snapchat-Structs.h>
@class NSArray, NSMutableArray;

@interface SCStoryMediaMessageViewModel : NSObject {

	long long _indexOfMediaForStoryThumbnail;
	NSArray* _mediaProviderList;
	NSMutableArray* _cachedStoryThumbnail;
	NSMutableArray* _storyThumbnailFetchingActions;
	BOOL _sentByUser;
	BOOL _isSent;
	BOOL _isSending;

}
-(BOOL)shouldDisplayActivityIndicator;
-(BOOL)shouldDisplaySendingOverlay;
-(BOOL)normallyRequiredMediaLoaded;
-(BOOL)minimumRequiredMediaLoaded;
-(void)fetchNextImageToDisplay:(CGSize)arg1 completionHandler:(/*^block*/id)arg2 ;
-(BOOL)containsMediaId:(id)arg1 ;
-(id)initWithStoryMediaMessage:(id)arg1 ;
-(id)initWithStoryMediaMessageV3:(id)arg1 currentUserName:(id)arg2 ;
-(void)_resetCachedStoryThumbnail;
-(BOOL)_requiredMediaLoaded:(long long)arg1 ;
-(void)_fetchNextCachedImage:(/*^block*/id)arg1 ;
-(BOOL)isLoadingFirstMedia;
-(BOOL)isProcessingOrUploadingMedia;
-(BOOL)anyMediaUploaded;
-(BOOL)allMediaLoaded;
-(void)dealloc;
-(void)_didReceiveMemoryWarning:(id)arg1 ;
-(BOOL)readyForDisplay;
@end

