/*
* This header is generated by classdump-dyld 1.0
* on Saturday, May 19, 2018 at 4:52:37 PM Eastern Daylight Time
* Operating System: Version 10.2 (Build 14C92)
* Image Source: /var/containers/Bundle/Application/7AB1B6B1-534C-4BF9-A7EB-BE2BD4C13043/Snapchat.app/Snapchat
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <Snapchat/SCGenericSettingsViewController.h>
#import <Snapchat/SCLagunaDeviceNameEditingViewDelegate.h>

@class UILabel, SCLagunaDeviceNameEditingView, SCUserSession, SCLagunaDevice, UIButton, UIActivityIndicatorView, NSString;

@interface SCLagunaSettingsChangeNameViewController : SCGenericSettingsViewController <SCLagunaDeviceNameEditingViewDelegate> {

	UILabel* _descriptionLabel;
	SCLagunaDeviceNameEditingView* _nameView;
	SCUserSession* _userSession;
	SCLagunaDevice* _device;
	UIButton* _saveBar;
	UIActivityIndicatorView* _saveBarActivity;
	BOOL _didUpdateDeviceName;

}

@property (readonly) unsigned long long hash; 
@property (readonly) Class superclass; 
@property (copy,readonly) NSString * description; 
@property (copy,readonly) NSString * debugDescription; 
-(void)leftSwipePrepare;
-(void)leftSwipeCancelled;
-(id)getTitle;
-(void)inputKeyboardWillChangeFrame:(id)arg1 ;
-(id)initWithDevice:(id)arg1 withUserSession:(id)arg2 ;
-(BOOL)_deviceNameExisted:(id)arg1 ;
-(void)createSaveBar;
-(void)setSaveBarTitle:(id)arg1 ;
-(void)saveButtonBarPressed;
-(void)_saveButtonBarPressed;
-(void)_alertNameTooShort;
-(void)_alertNameExisted:(id)arg1 ;
-(void)startSaveBarAnimation;
-(void)stopSaveBarAnimation;
-(void)dealloc;
-(void)loadView;
-(void)viewWillAppear:(BOOL)arg1 ;
-(void)viewWillDisappear:(BOOL)arg1 ;
-(void)textDidChange:(id)arg1 ;
-(void)viewDidLoad;
-(BOOL)textViewShouldReturn:(id)arg1 ;
@end

