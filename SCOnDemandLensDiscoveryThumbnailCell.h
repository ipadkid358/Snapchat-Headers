/*
* This header is generated by classdump-dyld 1.0
* on Saturday, May 19, 2018 at 4:52:38 PM Eastern Daylight Time
* Operating System: Version 10.2 (Build 14C92)
* Image Source: /var/containers/Bundle/Application/7AB1B6B1-534C-4BF9-A7EB-BE2BD4C13043/Snapchat.app/Snapchat
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <Snapchat/Snapchat-Structs.h>
#import <UIKit/UICollectionViewCell.h>
#import <Snapchat/SCLensDiscoveryCellProtocol.h>

@class UIImageView, UIView, SCLoadingIndicatorView, NSString;

@interface SCOnDemandLensDiscoveryThumbnailCell : UICollectionViewCell <SCLensDiscoveryCellProtocol> {

	UIImageView* _selectedIndicatorView;
	UIView* _contentContainer;
	SCLoadingIndicatorView* _loadingIndicator;
	UIImageView* _thumbnailImageView;
	long long _state;
	NSString* _thumbnailImageUrlString;

}

@property (assign,nonatomic) long long state;                                 //@synthesize state=_state - In the implementation block
@property (nonatomic,retain) NSString * thumbnailImageUrlString;              //@synthesize thumbnailImageUrlString=_thumbnailImageUrlString - In the implementation block
@property (readonly) unsigned long long hash; 
@property (readonly) Class superclass; 
@property (copy,readonly) NSString * description; 
@property (copy,readonly) NSString * debugDescription; 
-(id)selectedIndicatorView;
-(void)setLensDiscoveryCellInfo:(id)arg1 ;
-(void)setThumbnailImageUrlString:(NSString *)arg1 ;
-(NSString *)thumbnailImageUrlString;
-(void)loadThumbnailImageWithUrlString:(id)arg1 dataFetcher:(id)arg2 ;
-(id)initWithFrame:(CGRect)arg1 ;
-(long long)state;
-(void)setState:(long long)arg1 ;
-(void)prepareForReuse;
-(void)setIsSelected:(BOOL)arg1 ;
-(id)loadingIndicator;
-(id)thumbnailImageView;
@end

