/*
* This header is generated by classdump-dyld 1.0
* on Saturday, May 19, 2018 at 4:52:01 PM Eastern Daylight Time
* Operating System: Version 10.2 (Build 14C92)
* Image Source: /var/containers/Bundle/Application/7AB1B6B1-534C-4BF9-A7EB-BE2BD4C13043/Snapchat.app/Snapchat
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <Snapchat/GPBMessage.h>

@class NSString, SCMTStreamingMediaInfo, SCMTPublicMediaInfo, SCMTOneSnapInfo, GPBEnumArray;

@interface SCSCOREMediaInfo : GPBMessage

@property (assign,nonatomic) int mediaType; 
@property (assign,nonatomic) double mediaLengthSecs; 
@property (assign,nonatomic) int orientation; 
@property (assign,nonatomic) BOOL frontFacing; 
@property (assign,nonatomic) BOOL zipped; 
@property (nonatomic,copy) NSString * mediaIdMsm; 
@property (nonatomic,copy) NSString * mediaKey; 
@property (nonatomic,copy) NSString * mediaIv; 
@property (nonatomic,copy) NSString * thumbnailIv; 
@property (assign,nonatomic) BOOL isInfiniteDuration; 
@property (nonatomic,retain) SCMTStreamingMediaInfo * streamingMediaInfo; 
@property (assign,nonatomic) BOOL hasStreamingMediaInfo; 
@property (nonatomic,retain) SCMTPublicMediaInfo * publicMediaInfo; 
@property (assign,nonatomic) BOOL hasPublicMediaInfo; 
@property (nonatomic,retain) SCMTOneSnapInfo * streamingSnapInfo; 
@property (assign,nonatomic) BOOL hasStreamingSnapInfo; 
@property (assign,nonatomic) float aspectRatio; 
@property (nonatomic,copy) NSString * thumbnailDominantColor; 
@property (nonatomic,retain) GPBEnumArray * thumbnailVariantsArray; 
@property (nonatomic,readonly) unsigned long long thumbnailVariantsArray_Count; 
+(id)descriptor;
@end

