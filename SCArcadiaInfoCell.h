/*
* This header is generated by classdump-dyld 1.0
* on Saturday, May 19, 2018 at 4:52:23 PM Eastern Daylight Time
* Operating System: Version 10.2 (Build 14C92)
* Image Source: /var/containers/Bundle/Application/7AB1B6B1-534C-4BF9-A7EB-BE2BD4C13043/Snapchat.app/Snapchat
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <UIKit/UITableViewCell.h>

@protocol SCArcadiaInfoCellDelegate;
@class UILabel, UIButton, NSString;

@interface SCArcadiaInfoCell : UITableViewCell {

	UILabel* _titleLabel;
	UILabel* _idLabel;
	UIButton* _shareButton;
	id<SCArcadiaInfoCellDelegate> _delegate;

}

@property (assign,nonatomic,__weak) id<SCArcadiaInfoCellDelegate> delegate;              //@synthesize delegate=_delegate - In the implementation block
@property (nonatomic,copy) NSString * idText; 
@property (nonatomic,copy) NSString * titleText; 
-(void)didTapShareButton;
-(void)setIdText:(NSString *)arg1 ;
-(NSString *)idText;
-(id)initWithCoder:(id)arg1 ;
-(void)setDelegate:(id<SCArcadiaInfoCellDelegate>)arg1 ;
-(id)initWithStyle:(long long)arg1 reuseIdentifier:(id)arg2 ;
-(id<SCArcadiaInfoCellDelegate>)delegate;
-(void)commonInit;
-(void)setTitleText:(NSString *)arg1 ;
-(NSString *)titleText;
@end

