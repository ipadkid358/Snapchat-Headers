/*
* This header is generated by classdump-dyld 1.0
* on Saturday, May 19, 2018 at 4:52:43 PM Eastern Daylight Time
* Operating System: Version 10.2 (Build 14C92)
* Image Source: /var/containers/Bundle/Application/7AB1B6B1-534C-4BF9-A7EB-BE2BD4C13043/Snapchat.app/Snapchat
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <libobjc.A.dylib/NSCoding.h>

@class SCQueuePerformer, NSArray;

@interface SCRecentFriends : NSObject <NSCoding> {

	SCQueuePerformer* _performer;
	long long _capacity;
	NSArray* _recentFriends;

}

@property (retain) NSArray * recentFriends;              //@synthesize recentFriends=_recentFriends - In the implementation block
+(BOOL)shouldAddAsRecent:(id)arg1 ;
-(id)initWithPerformer:(id)arg1 ;
-(void)setPerformer:(id)arg1 ;
-(void)setRecentFriends:(NSArray *)arg1 ;
-(NSArray *)recentFriends;
-(void)updateWithChats:(id)arg1 ;
-(void)updateWithUsernames:(id)arg1 ;
-(void)updateWithUsername:(id)arg1 timestamp:(id)arg2 ;
-(void)removeFriend:(id)arg1 ;
-(void)_updateWithRecentFriends:(id)arg1 ;
-(void)_updateLastInteractionTimestampWithRecentFriend:(id)arg1 ;
-(id)initWithCoder:(id)arg1 ;
-(void)encodeWithCoder:(id)arg1 ;
-(id)recents;
-(unsigned long long)capacity;
-(void)setCapacity:(unsigned long long)arg1 ;
@end

